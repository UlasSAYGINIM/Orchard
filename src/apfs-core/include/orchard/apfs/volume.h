...
public:
  static blockio::Result<VolumeContext> Load(const blockio::Reader& reader,
                                             ...
                                             std::uint64_t xid_limit,
                                             const VolumeInfo& info,
                                             const OmapResolver& container_omap);

  [[nodiscard]] const VolumeInfo& info() const noexcept { return info_; }
  [[nodiscard]] std::uint32_t block_size() const noexcept { return block_size_; }
  [[nodiscard]] std::uint64_t xid_limit() const noexcept { return xid_limit_; }
  ...
  [[nodiscard]] blockio::Result<std::optional<XattrRecord>>
  FindXattr(std::uint64_t inode_id, std::string_view name) const;
  [[nodiscard]] blockio::Result<std::vector<std::uint8_t>>
  ReadPhysicalBytes(std::uint64_t physical_block_index,
                    std::uint64_t block_offset,
                    std::size_t size) const;

private:
  VolumeContext(const blockio::Reader& reader,
                std::uint64_t container_byte_offset,
                std::uint32_t block_size,
                std::uint64_t xid_limit,
                VolumeInfo info,
