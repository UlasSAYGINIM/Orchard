...
blockio::Result<FileMetadata> GetFileMetadata(const VolumeContext& volume, const std::uint64_t inode_id) {
  ...
  if (xattr_result.value().has_value()) {
    auto compression_result = ParseCompressionInfo(
        std::span<const std::uint8_t>(xattr_result.value()->data.data(),
                                      xattr_result.value()->data.size()));
...
blockio::Result<std::vector<std::uint8_t>> ReadFileRange(const VolumeContext& volume,
  ...
  if (!IsRegularFile(metadata_result.value().kind) && metadata_result.value().kind != InodeKind::kSymlink) {
  ...
  if (compression_xattr_result.value().has_value()) {
    ...
    return std::vector<std::uint8_t>(
        ...
        decoded_result.value().begin() +
            static_cast<std::ptrdiff_t>(offset + bounded_size));
  }
  ...
  for (const auto& extent : extents_result.value()) {
    ...
    const auto physical_block = extent.physical_block + (physical_byte_offset / volume.block_size());
    ...
    std::copy(data_result.value().begin(),
              data_result.value().end(),
