...
VolumeContext::VolumeContext(const blockio::Reader& reader,
                             ...
                             const std::uint32_t block_size,
                             const std::uint64_t xid_limit,
                             VolumeInfo info,
                             const std::uint64_t root_tree_block_index)
    : reader_(&reader),
      container_byte_offset_(container_byte_offset),
      block_size_(block_size),
      xid_limit_(xid_limit),
      info_(std::move(info)),
...
blockio::Result<std::size_t> VolumeContext::VisitFilesystemRecords(
...
blockio::Result<InodeRecord> VolumeContext::GetInode(const std::uint64_t inode_id) const {
  ...
  auto visit_result = VisitFilesystemRecords([&found, inode_id](const NodeRecordView& record)
                                                 -> blockio::Result<bool> {
...
VolumeContext::ListDirectoryEntries(const std::uint64_t directory_inode_id) const {
  ...
  auto visit_result =
      VisitFilesystemRecords([&entries, directory_inode_id](const NodeRecordView& record)
                                 -> blockio::Result<bool> {
        auto key_result = ParseFsKeyHeader(record.key);
        if (!key_result.ok()) {
          return key_result.error();
  ...
  std::sort(entries.begin(), entries.end(), [this](const DirectoryEntryRecord& left,
                                                   const DirectoryEntryRecord& right) {
    const auto left_name = info_.case_insensitive ? AsciiFold(left.key.name) : left.key.name;
    const auto right_name = info_.case_insensitive ? AsciiFold(right.key.name) : right.key.name;
...
VolumeContext::ListFileExtents(const std::uint64_t inode_id) const {
  ...
  auto visit_result = VisitFilesystemRecords([&extents, inode_id](const NodeRecordView& record)
                                                 -> blockio::Result<bool> {
    auto key_result = ParseFsKeyHeader(record.key);
    if (!key_result.ok()) {
      return key_result.error();
  ...
  std::sort(extents.begin(), extents.end(), [](const FileExtentRecord& left,
                                               const FileExtentRecord& right) {
...
blockio::Result<std::optional<XattrRecord>> VolumeContext::FindXattr(const std::uint64_t inode_id,
                                                                     const std::string_view name) const {
  std::optional<XattrRecord> found;
  auto visit_result =
      VisitFilesystemRecords([&found, inode_id, name](const NodeRecordView& record)
                                 -> blockio::Result<bool> {
        auto key_result = ParseFsKeyHeader(record.key);
        if (!key_result.ok()) {
          return key_result.error();
...
blockio::Result<std::vector<std::uint8_t>> VolumeContext::ReadPhysicalBytes(
    const std::uint64_t physical_block_index,
    const std::uint64_t block_offset,
    const std::size_t size) const {
  auto absolute_offset_result =
      ComputeAbsoluteByteOffset(container_byte_offset_, block_size_, physical_block_index, block_offset);
  if (!absolute_offset_result.ok()) {
    return absolute_offset_result.error();
  }

  return blockio::ReadExact(*reader_,
                            blockio::ReadRequest{.offset = absolute_offset_result.value(),
                                                 .size = size});
}
