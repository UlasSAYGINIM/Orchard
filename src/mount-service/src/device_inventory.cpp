...
DeviceInventory::DiffAgainst(const std::vector<DeviceInterfaceInfo>& present_interfaces) const {
  ...
  for (const auto& device : present_interfaces) {
    ...
    const auto existing = std::find_if(devices_.begin(), devices_.end(), [&](const DeviceEntry& entry) {
  ...
  for (const auto& entry : devices_) {
    const auto present = std::find_if(
        present_interfaces.begin(), present_interfaces.end(), [&](const DeviceInterfaceInfo& device) {
...
std::optional<KnownDeviceRecord> DeviceInventory::FindDevice(const std::wstring_view device_path) const {
...
std::optional<KnownDeviceRecord> DeviceInventory::RemoveDevice(const std::wstring_view device_path) {
  const auto normalized_path = NormalizeDevicePathKey(device_path);
  const auto existing = std::find_if(devices_.begin(), devices_.end(), [&](const DeviceEntry& entry) {
    return entry.normalized_path == normalized_path;
  });
...
std::vector<KnownDeviceRecord> DeviceInventory::ListDevices() const {
  ...
  std::sort(devices.begin(), devices.end(),
            [](const KnownDeviceRecord& left, const KnownDeviceRecord& right) {
              return NormalizeDevicePathKey(left.device_path) < NormalizeDevicePathKey(right.device_path);
            });
  return devices;
}

void DeviceInventory::AttachMount(const std::wstring_view device_path, const std::uint64_t volume_object_id,
...
std::size_t DeviceInventory::MountedVolumeCountForDevice(const std::wstring_view device_path) const {
  const auto normalized_path = NormalizeDevicePathKey(device_path);
  const auto existing = std::find_if(devices_.begin(), devices_.end(), [&](const DeviceEntry& entry) {
    return entry.normalized_path == normalized_path;
  });
  ...
  return static_cast<std::size_t>(std::count_if(
      existing->record.volumes.begin(), existing->record.volumes.end(),
